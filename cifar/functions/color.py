...
from zeno import distill, ZenoOptions, DistillReturn
...
def red_count(df, ops: ZenoOptions):
    ...
    return DistillReturn(distill_output=[red_pixels(im) for im in imgs])


@distill
def blue_border_count(df, ops):
    imgs = [Image.open(os.path.join(ops.data_path, img)) for img in df[ops.data_column]]
    return DistillReturn(distill_output=[blue_border_pixels(im) for im in imgs])
